...
public class AvlData implements Serializable {
	 ...
	 * 
	 * @param timestamp  Timestamp when data was acquired
	 * @param gpsElement GPS information
	 * @param ioElement  Input output information
	 */
	...
	/**
	 * @param inputOutputElement The inputOutputElement to set.
	 */
	...
	public String toString() {
		return "FM3 [Priority=" + getPriority() + "] [GPS element=" + getGpsElement() + "] [IO="
	 ...
	 * @throws IOException 
	 */
	public byte[] encode(AvlData[] datas) throws CodecException, IOException {
		if (datas.length > 255) {
...
//			byte[] result = codecOutputStream.getByteArray();
	...
	public int encode(AvlData[] datas, int length, byte[] buffer, int offsetInBuffer) throws CodecException, IOException {
		if (length > 255) {
			throw new CodecException("Maximum elements is 255");
		}
		...
		try {
			...
			dataOutputStream.writeByte(length);
			dataOutputStream.flush();
//			int result = codecOutputStream.getWritenDataLength();
	 ...
	 */
	public AvlData[] decode(byte[] dataByteArray) throws CodecException, IOException {
