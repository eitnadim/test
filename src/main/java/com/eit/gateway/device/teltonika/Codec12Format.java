...
public class Codec12Format {
	...
	public static String asciiToHex(String ascii) {
		...
		for (int i = 0; i < ascii.length(); i++) {
			hex.append(Integer.toHexString(ascii.charAt(i)).length() < 2 ? '0' + Integer.toHexString(ascii.charAt(i))
					: Integer.toHexString(ascii.charAt(i)));
		}
		return hex.toString();
	}
	...
	public static String prepareCommand(String gprsCommand) {
		...
		String crc = String.format("%08x", CommonUtils
				.getCrc(hexStringToByteArray((temp.append(commandLength).append(command).append("01")).toString())));
