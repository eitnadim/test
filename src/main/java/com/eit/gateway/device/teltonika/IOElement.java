...
public class IOElement {
	 ...
	 * @return Returns property associated with id, if property not found null is
	 *         returned
	 */
	...
	/**
	 * Adds property to io element. If element already contains property, overwrite
	 * it.
	 * 
	 * @param property Property to add
	 * @throws UnacceptablePropertyException If binary property with higher than 255
	 *                                       id is added or id is negative
	 */
	public void addProperty(int[] property) throws IllegalArgumentException {
		if (property[0] >  65535 || property[0] < 0) {
	 ...
	 * @throws IOException Thrown by passed input stream, or if unable to build io
		...
		try (DataInputStream dataInputStream = new DataInputStream(codecInputStream);) {
			...
			while (propertiesRead < properties) {
				if (codecInputStream.readBits(1) == 1) {
					...
					switch (type) {
					case BOOLEAN_TYPE: {
						result.setPropertyValue(propertiesIndex,
								(codecInputStream.readBits(1) == 0) ? Integer.MIN_VALUE : Integer.MAX_VALUE);
						break;
					}
					case INTEGER_TYPE: {
	...
	public static void write(BitOutputStream codecOutputStream, IOElement ioElement) throws IOException {
		try(DataOutputStream dataOutputStream = new DataOutputStream(codecOutputStream);) {
	...
	/**
	 * This method writes property value to array. If this property's id is in id's
	 * array, new value is written to values array, if not, id is written to first
	 * free array index. Free array indexes is that one that contains zero. If array
	 * is full (no free indexes) array is extended.
	 * 
	...
	public int hashCode() {
		final int prime = 31;
		int result = 1;
		result = prime * result + Arrays.hashCode(propertyIds);
		result = prime * result + Arrays.hashCode(propertyValues);
		result = prime * result + Objects.hash(propertiesCount);
		return result;
	}	
